...
def get_signals():
    conn = get_db()
    signals = conn.execute("SELECT * FROM signals WHERE status = 'pending' ORDER BY created_at DESC").fetchall()
    conn.close()
    result = []
    for s in signals:
        row = dict(s)
...
def get_stats():
    ...
    total_profit = conn.execute("SELECT COALESCE(SUM(profit), 0) as p FROM bets WHERE status IN ('won','lost')").fetchone()['p']
    total_staked = conn.execute("SELECT COALESCE(SUM(stake), 0) as s FROM bets WHERE status IN ('won','lost')").fetchone()['s']
    win_rate = round((won / total * 100), 1) if total > 0 else 0
    roi = round((total_profit / total_staked * 100), 1) if total_staked > 0 else 0
    bankroll = conn.execute("SELECT amount FROM bankroll ORDER BY id DESC LIMIT 1").fetchone()
    current_bankroll = bankroll['amount'] if bankroll else 1000
    history = conn.execute("SELECT amount, updated_at FROM bankroll ORDER BY updated_at ASC").fetchall()
    conn.close()
    return jsonify({"total_bets": total, "open_bets": open_bets, "won": won, "lost": lost, "win_rate": win_rate, "roi": roi, "total_profit": round(total_profit, 2), "total_staked": round(total_staked, 2), "current_bankroll": round(current_bankroll, 2), "bankroll_history": [{"amount": h['amount'], "date": h['updated_at']} for h in history]})
